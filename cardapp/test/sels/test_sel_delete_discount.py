...
def test_delete_fail(driver):
    ...
    e = driver.find_element(
        By.CSS_SELECTOR,
        "table tbody tr:first-child .list-buttons-column form button span"
    )
    e.click()
    time.sleep(2)
    alert = Alert(driver)
    alert.accept()
    e = driver.find_element(By.CSS_SELECTOR, "body > div.container > div.alert.alert-danger.alert-dismissable")
    driver.save_screenshot(r"D:\ProjectGithub\Card-Selling-System\cardapp\test\sels\ActualResult\delete_fail.png")
    assert 'TỪ CHỐI XÓA' in e.text
    driver.save_screenshot(r"D:\ProjectGithub\Card-Selling-System\cardapp\test\sels\ActualResult\delete_fail.png")